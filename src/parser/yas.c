...
void print_code(FILE *out, int pos)
{
    ...
    */
    ...
    /* Printing format:
       ...
       cccccccccccc is code
    */
    unsigned char outstring[26];
    if (tcount) {
	int i, j = 0;
	//strcpy(outstring, "  0x000:              | ");
	//hexstuff(outstring, pos, 3);
	for (i = 0; i < bcount; i++){
        //printf("Entered loop: %d, code: %x\n", bcount, code[i]&0xFF);
        char x[3];
	    hexstuff(x, code[i]&0xFF, 2);
        x[2] = '\0';
        //printf("%c %c ", x[0], x[1]);
        unsigned char buf = (unsigned char)(strtol(x, NULL, 16) & 0xFF);
        //printf("%x ", buf);
        //fprintf(out, "%x", (unsigned char)(code[i] & 0xFF));
        //fwrite(&buf, 1, sizeof(unsigned char), out);
        outstring[j++] = buf;
    }
    i = 0;
    while (i < j)
    {
        unsigned char buf1 = outstring[i++];
        fwrite(&buf1, 1, sizeof(unsigned char), out);

        /*if ( i < j) {
        unsigned char buf2 = outstring[i++];
        fwrite(&buf2, 1, sizeof(unsigned char), out);
        }*/
    }
